...
namespace PowerSDR
{
	public class FLEX5000RX2CalForm : System.Windows.Forms.Form
	{
		...
		private void TestGenBal()
		{
			...
			if(Math.Abs(-26.6 - adc_l) > 1.0f && Math.Abs(-31.8 - adc_l) > 1.0f)
			{
				lstDebug.Items.Insert(0, " Gen/Bal Test: Failed ADC_L -31.8+/-1.0 ("+adc_l.ToString("f1")+")");
				test_genbal += "ADC_L -31.8+/-1.0 ("+adc_l.ToString("f1")+")\n";
				b = false;
			}
			if(Math.Abs(-26.1 - adc_r) > 1.0f && Math.Abs(-31.8 - adc_r) > 1.0f)
			{
				lstDebug.Items.Insert(0, " Gen/Bal Test: Failed ADC_R -31.8+/-1.0 ("+adc_r.ToString("f1")+")");
				test_genbal += "ADC_R -31.8+/-1.0 ("+adc_r.ToString("f1")+")\n";
				b = false;
			}
		...
		unsafe private void TestRX2Filters()
		{
			...
			for(int i=0; i<band_freqs.Length; i++)
			{
				...
				if(do_band)
				{
					...
					if((resistor && Math.Abs((off-on) - avg[i]) > tol) ||
						!resistor && Math.Abs((off-on) - avg2[i]) > tol)
					{
			...
			Debug.WriteLine("");
				...
				if(test_rx2_filters.StartsWith("RX2 Filter Test: Failed ("))
					test_rx2_filters = test_rx2_filters.Substring(0, test_rx2_filters.Length-2)+")";
